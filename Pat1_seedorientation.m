...
seed_location_preplan = dicominfo('C:\Users\SA\Desktop\0531\Thesis\Data\Pat1_Preplan\PL001.dcm','dictionary',D);
...
seed_location_iotp = dicominfo('C:\Users\SA\Desktop\0531\Thesis\Data\Pat1_IntraoperativeTrementPlan\PL001.dcm','dictionary',D);
...
% y = [0.941 0.972 1.013 1.033 1.022 1.000 0.937 0.856 0.691 0.540 0.415 0.314 0.236 0.176 0.131 0.00969];
plot(x,y,'--')
hold on
...
for i = 1:length(tplan)
   ...
   %find closest z position 
    ...
    slice = slice(1) % in case it locate to middle of two slice, we have to select just one from them
   %find the point on z position plan 
    r = find(abs(dose_r-y) == min(abs(dose_r-y)));
    c = find(abs(dose_c-x) == min(abs(dose_c-x)));
    disp([c,r,slice])
    
    npoint = 5; % select other n point from center
    
    disp(flip(double(dose(r,c:c+npoint,slice))))
    yD = flip(double(dose(r,c:c+npoint,slice))); % Coordinates depend on the Dose Map, must be chosen manualy
...
xlabel('mm')
ylabel('G(r)')
title('IntraoperativeTrementPlan_1 total seed')
